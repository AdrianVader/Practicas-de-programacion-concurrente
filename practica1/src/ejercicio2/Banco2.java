...
class Cliente extends Thread {
  ...
  /*sincronizamos la cuenta que nos pasan, para acceder de forma única, sumando la cantidad sin machacar valores*/
  ...
  /*sincronizamos la cuenta para consultar y modificar el valor sin que otro lo toque. Mantiene en espera el hilo si no hay suficiente*/
  protected void hacer_reintegro(int caj, int cue, int cantidad) {
	  ...
	  synchronized(Banco2.cuentas[cue]){
		  while(Banco2.cuentas[cue].saldo(Banco2.cajeros[caj]) < cantidad){
			  ...
			  try {
				  System.out.println("Cliente " + this.id + ": reintegro de " + cantidad +
		           " Euros en cuenta #" + Banco2.cuentas[cue].numCuenta(Banco2.cajeros[caj]) +
		           " con cajero " + Banco2.cajeros[caj].id + " no se puede realizar por saldo insuficiente, espera");
		  ...
		  if(espera)
			  System.out.println("Cliente " + this.id + ": reintegro de " + cantidad +
			   " Euros en cuenta #" + Banco2.cuentas[cue].numCuenta(Banco2.cajeros[caj]) +
			   " con cajero " + Banco2.cajeros[caj].id + " sale de espera, ya hay saldo suficiente");
...
class Cliente0 extends Cliente {
  ...
  public void run() {
    ...
    try {
    	...
    	System.err.println("Interrumpido mientras dormido");
    }
